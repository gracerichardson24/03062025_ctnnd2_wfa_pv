...
Image_data = read.csv("DATA/Summary_set1.csv")
Image_data = read.csv("DATA/Summary_set2.csv")
#Decoder = read.csv("Data_all_sets/Decoder_all_sets.csv")
```
...
```{r}
Image_data = read_csv("Data_all_sets/Summary_all_sets.csv")
Decoder = read_csv("Data_all_sets/Decoder_all_sets.csv")

# Step 1: Split the string at " - " and take the second half
Image_data$Image <- sapply(strsplit(Image_data$Image, " - "), function(x) x[2])
Image_data$Animal <- str_extract(Image_data$Image, "(?<=set[12]_).{6}")
Image_data_decoded <- merge(Image_data, Decoder, by = "Animal")
Image_data <- Image_data_decoded
...
ReplicateAverages_PV <- Image_data |>
...
TotalAverages_PV$Genotype = sapply(strsplit(TotalAverages_PV$Genotype_age, " "), function(x) x[1])
TotalAverages_PV
TotalAverages_PV$Genotype <- factor(TotalAverages_PV$Genotype, levels = c("WT", "HET", "KO"))

Image_data$Genotype_age <- factor(Image_data$Genotype_age, levels = c("WT P29",  "HET P29", "KO P29", "WT P30", "HET P30", "KO P30"))
Image_data

# Plots Superplot based on biological replicate averages
superplot_PV <- ggplot() +
  # Plot original data points with `Green Puncta Count`
  geom_quasirandom(data = Image_data, aes(x = Genotype_age, y = `Green Puncta Count`), color = "black", size = 2, alpha = 1/2) +
  # Bar plot for total averages
  geom_bar(data = TotalAverages_PV, aes(x = Genotype_age, y = PV_avg_avg, fill = Genotype), stat = "identity", width = 0.8, alpha = 0.5) +
...
```{r}
modelagePV = lm(`Green Puncta Count` ~ Age + Genotype + Age:Genotype,
           data = Image_data)
summary(modelagePV)
...
```{r}
# PV statistics
modelPV = lm(`Green Puncta Count` ~ Genotype,
           data = Image_data_P29)
...
DunnettTest(`Green Puncta Count` ~ Genotype,
            data = Image_data_P29,
            control = "WT")
```
...
```{r}
Image_data$Genotype_age <- paste(Image_data$Genotype, Image_data$Age)
Image_data_P29 <- Image_data[Image_data$Age == "P29",]

ReplicateAverages_PV <- Image_data_P29 |>
...
TotalAverages_PV$Genotype <- factor(TotalAverages_PV$Genotype, levels = c("WT", "HET", "KO"))
Image_data_P29$Genotype <- factor(Image_data_P29$Genotype, levels = c("WT", "HET", "KO"))
...
superplot_PV <- ggplot() +
  # Plot original data points with `Green Puncta Count`
  geom_quasirandom(data = Image_data_P29, aes(x = Genotype, y = `Green Puncta Count`, color = Genotype), size = 2, alpha = 1/2) +
  # Bar plot for total averages
  geom_bar(data = TotalAverages_PV, aes(x = Genotype, y = PV_avg_avg, fill = Genotype), fill = "lightgray", color = "black", stat = "identity", width = 0.8, alpha = 0.5) +
  # Error bars for replicate averages
  stat_summary(data = ReplicateAverages_PV, aes(x = Genotype, y = PV_avg), fun.data = mean_se, geom = "errorbar", width = 0.1, color = "black") +
  # Points for replicate averages
...
```{r}
modelPNN = lm(`Red Puncta Count` ~ Genotype,
           data = Image_data_P29)
...
DunnettTest(`Red Puncta Count` ~ Genotype,
            data = Image_data_P29,
            control = "WT")
```
...
```{r}
Image_data_P29$Genotype <- factor(Image_data_P29$Genotype, levels = c("WT", "HET", "KO"))
ReplicateAverages_WFA <- Image_data_P29 |>
  group_by(Genotype, Animal) |>
...
Image_data_P29$Genotype_age <- factor(Image_data_P29$Genotype_age, levels = c("WT P29",  "HET P29", "KO P29"))
Image_data_P29
...
superplot_WFA <- ggplot() +
  # Plot original data points with `Green Puncta Count`
  geom_quasirandom(data = Image_data_P29, aes(x = Genotype, y = `Red Puncta Count`), color = "black", size = 2, alpha = 1/2) +
  # Bar plot for total averages
  geom_bar(data = TotalAverages_WFA, aes(x = Genotype, y = WFA_avg_avg, fill = Genotype), stat = "identity", width = 0.8, alpha = 0.5) +
  # Error bars for replicate averages
  stat_summary(data = ReplicateAverages_WFA, aes(x = Genotype, y = WFA_avg), fun.data = mean_se, geom = "errorbar", width = 0.1, color = "black") +
  # Points for replicate averages
  geom_point(data = ReplicateAverages_WFA, aes(x = Genotype, y = WFA_avg), color = "black", size = 4, alpha = 0.5) +
  # Theme and labels
  theme(
    legend.text = element_text(size = 6),
    legend.title = element_text(size = 8)
...
```{r}
model_double_pos = lm(`Colocalized Puncta Count` ~ Genotype,
           data = Image_data_P29)
...
DunnettTest(`Colocalized Puncta Count` ~ Genotype,
            data = Image_data_P29,
            control = "WT")
```
...
```{r}
Image_data_P29$Genotype <- factor(Image_data_P29$Genotype, levels = c("WT", "HET", "KO"))
ReplicateAverages_double_pos <- Image_data_P29 |>
  group_by(Genotype, Animal) |>
...
df_p_val <- data.frame(
  ...
  group2 = c("KO", "KO", "WT"),
  label = c("ns", "ns", "ns"),
  y.position = c(25, 40, 32)
)

# Plots Superplot based on biological replicate averages
superplot_double_pos <- ggplot() +
  # Plot original data points with `Green Puncta Count`
  geom_quasirandom(data = Image_data_P29, aes(x = Genotype, y = `Colocalized Puncta Count`, color = Genotype), size = 2, alpha = 1/2) +
  # Bar plot for total averages
  geom_bar(data = TotalAverages_double_pos, aes(x = Genotype, y = double_pos_avg_avg), fill = "lightgray", color = "black", stat = "identity", width = 0.8, alpha = 0.5) +
  # Error bars for replicate averages
  stat_summary(data = ReplicateAverages_double_pos, aes(x = Genotype, y = double_pos_avg), fun.data = mean_se, geom = "errorbar", width = 0.1, color = "black") +
  # Points for replicate averages
  geom_point(data = ReplicateAverages_double_pos, aes(x = Genotype, y = double_pos_avg, color = Genotype), size = 4, alpha = 0.5) +
  # Theme and labels
  theme(
    legend.text = element_text(size = 6),
    legend.title = element_text(size = 8)
...
ggsave(filename = "./Plots_all_sets/p29_Ctnnd2_superplot_double_pos.png", width =5, height = 5)

superplot_double_pos
```

```{r}
Image_data_P29$single_pos <- Image_data_P29$`Green Puncta Count` - Image_data_P29$`Colocalized Puncta Count`
model_single_pos = lm(single_pos ~ Genotype,
           data = Image_data_P29)
...
ReplicateAverages_single_pos <- Image_data_P29 |>
  group_by(Genotype, Animal) |>
...
superplot_single_pos <- ggplot() +
  # Plot original data points with `Green Puncta Count`
  geom_quasirandom(data = Image_data_P29, aes(x = Genotype, y = (single_pos), color = Genotype), size = 2, alpha = 1/2) +
  # Bar plot for total averages
  geom_bar(data = TotalAverages_single_pos, aes(x = Genotype, y = single_pos_avg_avg), fill = "lightgray", color = "black", stat = "identity", width = 0.8, alpha = 0.5) +
  # Error bars for replicate averages
  stat_summary(data = ReplicateAverages_single_pos, aes(x = Genotype, y = single_pos_avg), fun.data = mean_se, geom = "errorbar", width = 0.1, color = "black") +
  # Points for replicate averages
  geom_point(data = ReplicateAverages_single_pos, aes(x = Genotype, y = single_pos_avg, color = Genotype), size = 4, alpha = 0.5) +
  # Theme and labels
  theme(
    legend.text = element_text(size = 6),
    legend.title = element_text(size = 8)
