...
intensity_mean_plot <- ggplot() +
  ...
  geom_jitter(data = FluoData_set23_df, aes(x = genotype, y = Mean, color = animal),
...
# Save
ggsave("Intensity_plot_mean.png", plot = intensity_mean_plot, width = 2, height = 4, dpi = 300)
...
intensity_intden_plot <- ggplot() +
  ...
  geom_jitter(data = FluoData_Animal_IntDen, aes(x = genotype, y = mean_Intden, color = genotype),
              width = 0.2, size = 2, alpha = 0.9) +
  geom_jitter(data = FluoData_set23_df, aes(x = genotype, y = IntDen, color = animal),
              width = 0.2, size = 0.5, alpha = 0.6) +
  theme_minimal() +
  xlab(" ") +
  ylab("Integrated Density Intensity") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        legend.position = "bottom") +
  scale_x_discrete()

# Save
ggsave("Intensity_plot_intden.png", plot = intensity_intden_plot, width = 2, height = 4, dpi = 300)
